defmodule PhoenixStarter.ConnCase do
  ...
  using do
    quote do
      ...
      def guardian_login(%PhoenixStarter.User{} = user), do: guardian_login(conn(), user, :token, [])
      def guardian_login(%PhoenixStarter.User{} = user, token), do: guardian_login(conn(), user, token, [])
      def guardian_login(%PhoenixStarter.User{} = user, token, opts), do: guardian_login(conn(), user, token, opts)
